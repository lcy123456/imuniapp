<template>
    <view class="chat_header">
        <CustomNavBar :show-left="false" bg-color="#eff1f4">
            <template slot="center">
                <text v-if="storeConnectingStatus" class="header-text">
        ...
        >
            <view
                :style="{
                    top: popMenuPosition.top,
                    right: popMenuPosition.right
                }"
                class="more_menu"
            >
                <view class="arrow" />
...
export default {
    ...
    computed: {
        ...mapGetters(['storeSelfInfo', 'storeConnectingStatus'])
    },
    methods: {
        clickMenu({ idx }) {
            switch (idx) {
        ...
        async showMore() {
            const { right, bottom } = await getEl.call(this, '.more_icon');
            this.popMenuPosition.right =
                uni.getWindowInfo().windowWidth - right - 5 + 'px';
...
.chat_header {
    ...
    .more_menu {
        ...
        .arrow {
            ...
            border-style: solid;
            border-color: transparent transparent $uni-bg-color-black
                transparent;
        }
