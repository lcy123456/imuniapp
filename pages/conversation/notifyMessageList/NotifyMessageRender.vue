<template>
    <view :id="`auchor${source.clientMsgID}`" class="message_item">
        <my-avatar
            size="42"
            :desc="notifyContent.notificationName"
            :src="notifyContent.notificationFaceURL"
        />
        <view class="message_container">
            ...
            <view class="message_content_wrap">
                <view class="notify_message_container">
                    ...
                    <view class="notify_text" @click="checkUrl">
                        {{ notifyContent.text }}
                    </view>
...
export default {
    ...
    computed: {
        notifyContent() {
            ...
            try {
                notificationContent = JSON.parse(
                    this.source.notificationElem.detail
                );
            } catch (e) {
        ...
        getMediaSource() {
            if (
                this.notifyContent.mixType === NotificationMixTypes.TextAndImage
            ) {
                const picEl = this.notifyContent.pictureElem;
                const url =
                    picEl.SourcePicture?.Url ?? picEl.sourcePicture?.url;
                return url ?? '';
            }
            if (
                this.notifyContent.mixType === NotificationMixTypes.TextAndVideo
            ) {
